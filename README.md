...
核心原则：脚本不代替阅读，也不生成最终日报。脚本只提供候选池和可解释的检索线索，最终推荐与总结由 Codex 完成。
...
3. 在 Codex 中创建定时自动化任务，例如每天早上运行一次。

`Paper_Reader.txt` 默认被 `.gitignore` 忽略，因为它通常包含机器相关路径和私有筛选偏好。公开仓库只保留 `Paper_Reader.template.txt`。
...
- 抓取的原始数据
- 生成的日报
- 含有机器相关路径或私有筛选偏好的本地 prompt

根目录 `.gitignore` 已经为这些内容做了默认忽略。
...
目前尚未选择开源许可证。公开发布前建议添加 MIT、Apache-2.0 或其他合适许可证。
